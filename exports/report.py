...
Responsibilities
----------------
This module is the high-level report orchestrator.

IMPORTANT DESIGN RULE
---------------------
The Excel workbook is the SOURCE OF TRUTH for the complete report.

The complete PDF generated by this module therefore mirrors the Excel
workbook structure:
...
The short PDF score summary remains delegated to exports.pdf.

The complete PDF is intentionally generated here instead of calling
pdf.export_full_report(), because the latter represents a separate PDF
presentation and should not be the source of truth for the full report.

Branding
--------
The PDF header contains BOTH:
    - JESA logo on the left
    - ENSAM logo on the right

Logo lookup is robust and searches several common asset locations.
"""
...
def _find_logo(directory: Path, candidates: Sequence[str]) -> Optional[Path]:
    """
    Find a logo using exact names first and case-insensitive fallback second.
    """
...
def _locate_logos() -> Dict[str, Optional[Path]]:
    ...
    This fixes the common issue where the Excel/PDF generator expects a
    specific filename or directory while the actual asset uses another
    capitalization/name.
    """
...
class CompletePDFReportExporter:
    ...
    The order and content intentionally follow ExcelExporter.SHEETS.
    """
    ...
    def _build_executive_summary(
        ...
        # --------------------------------------------------------------
        # Executive KPI table EXACTLY reflecting Excel information
        # --------------------------------------------------------------
    ...
    def _build_recommendations(
        ...
        # Excel has a fallback using roadmap when no priority recommendations
        # exist. Keep exactly the same behavior.
        if not rows and roadmap:
            ...
            for phase in roadmap:
                ...
                for item in items:
                    ...
                    if recommendation_id.startswith(
                        "NO-REC-"
                    ):
                        continue
...
class ReportGenerator:
    ...
    PDF score summary:
        delegates to exports.pdf

    Complete PDF:
        generated from the same structures as ExcelExporter

    Excel:
        generated through ExcelExporter
    ...
    def __init__(
        ...
        self.complete_pdf_exporter = CompletePDFReportExporter(
            self.config
        )
    ...
    def generate_full_report(
        ...
        """
        Generate requested report deliverables.

        IMPORTANT:
            pdf_full is now the complete PDF equivalent of Excel.
        """
        ...
        # --------------------------------------------------------------
        # COMPLETE PDF = Excel equivalent
        # --------------------------------------------------------------

        if include_pdf_full:

            outputs["pdf_full"] = (
                self._generate_pdf_full(
                    report_data,
                    output_dir,
                )
            )
...
def generate_report(
    ...
    Supported values:
        ...
        pdf_full
            COMPLETE PDF equivalent of Excel.
        ...
        pdf
            Backward-compatible alias for pdf_score.

        report
            Backward-compatible alias for pdf_full.
    """
